...
def datacrawl(jobname, jobnum):
    input_job = jobname
    input_num = jobnum
    url = "https://xiaoyuan.zhaopin.com/full/0/0_0_0_0_0_-1_{}_{}_0"  # 1.导入包并准备网址
    con = pymysql.connect("localhost", "root", "root", "pythonDB")
    db = con.cursor()
    for i in range(1, input_num + 1):
        f = requests.get(url.format(input_job, i)).text  # 2.爬取 requests.get(url)---->text
        ...
        for div in divlist:  # 3.2 遍历内容
            ...
            sql = "insert into jobinfo(jobname,city,neednum,company,cptype,pubtime) values('%s','%s','%s','%s','%s','%s')" % (job, city, num, cmp, cmptype, pubtime)
            db.execute(sql)
            con.commit()
    db.close()
    con.close()